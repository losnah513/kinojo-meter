...
if ($launcher.codeSignatureRequired -ne $true -or [String]::IsNullOrWhiteSpace([string]$launcher.publisherSubject)) {
    throw 'Launcher Authenticode requirement is missing.'
}
if ($core.publicDistribution -ne $false -or $core.storageBucket -ne 'meter-core-private' -or $core.codeSignatureRequired -ne $true) {
