...
namespace KinojoMeterLauncher
{
    ...
    internal static class PrivateRuntimeProcessPlanBuilder
    {
            ...
            ActiveProtocolModuleState protocol)
        {
            ...
            string captureAssembly = null;
            if (capture != null)
            {
                if (!String.Equals(capture.Channel, runtime.Channel, StringComparison.Ordinal) ||
                    !String.Equals(capture.RuntimeBundleRevision, runtime.RuntimeBundleRevision, StringComparison.Ordinal) ||
                    !String.Equals(capture.RuntimeBundleLockSha256, runtime.RuntimeBundleLockSha256, StringComparison.Ordinal) ||
                    !String.Equals(capture.RuntimeModuleSetHash, runtime.RuntimeModuleSetHash, StringComparison.Ordinal) ||
                    !String.Equals(capture.ParentPrivateRuntimeVersion, runtime.ModuleVersion, StringComparison.Ordinal) ||
                    !String.Equals(capture.ParentPrivateRuntimeSha256, runtime.PackageSha256, StringComparison.Ordinal) ||
                    capture.ParentPrivateRuntimePointerGeneration != runtime.PointerGeneration ||
                    !String.Equals(capture.PrimaryArtifact, "KINOJO.Meter.Capture.dll", StringComparison.Ordinal))
                    throw new InvalidOperationException("Capture override가 private runtime의 exact parent/Bundle identity와 일치하지 않습니다.");
                captureAssembly = Path.GetFullPath(Path.Combine(capture.StagedDirectory, capture.PrimaryArtifact));
